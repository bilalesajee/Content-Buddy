...
<!--            <li class="active">
            ...
            <?php $filter = Request::segment(2) ; ?>
            <?php if(isset($filter) && $filter !='') { ?>
            <li>
                <?php if($filter == 'active'){?>
                    <strong>Active Users</strong>
                <?php }else if($filter == 'inactive'){ ?>
                    <strong>Pending Users</strong>
                <?php }else{ ?>
                     <strong>Blocked Users</strong>
                <?php } ?>
                <!--<strong><a href="<?=url('manage_users')?>">Show All</a></strong>-->
            </li>
            <?php }else{ ?>
            <li>
                        ...
                        <table class="table table-striped table-bordered table-hover dataTables-example" id="posts" data-page-size="15">
                            <thead>
                                <tr>
                                    ...
                                    <th  class="footable-visible text_center pad_right">Image<span class="footable-sort-indicator"></span></th> 
                                    <!--<th class="footable-visible text_center">Created At<span class="footable-sort-indicator"></span></th>-->
                                    <!--<th class="footable-visible text_center pad_right">Mail Verified</th>-->
                                    <?php if($title != 'Active Users'){ ?>
                                    <th class="footable-visible text_center pad_right">Approved</th>
                                    <?php } ?>
                                    <?php if($title != 'Inactive Users'){ ?>
                                        <th class="footable-visible text_center pad_right">Blocked</th>
                                    <?php } ?>
                                    <th class="footable-visible text_center pad_right">Joined at</th>
                                ...
                                if ($result) {
                                    foreach ($result as $key=>$post) {
                                        ...
                                        <tr id="row_<?= $id; ?>" >
                                            ...
                                            <td><?= $last_name; ?></td> 
                                            <td><?= $email; ?></td> 
                                            <td class="text_center"><?= $image; ?></td> 
                                            <!--<td class="text_center"><?= $created_at; ?></td>--> 
                                            <!--<td class="text_center"><input type="checkbox" data-id="<?=$post->id;?>" class="status" name="status" value="1" <?=($post->is_active == 1)?'checked':'';?>></td>--> 
                                            <?php if($title != 'Active Users'){ ?>
                                                <td class="text_center"><input type="checkbox" data-id="<?=$post->id;?>" class="approve_user" name="is_approved_by_admin" value="1" <?=($post->is_approved_by_admin == 1)?'checked':'';?>></td> 
                                            <?php } ?>
                                                
                                    <?php if($title != 'Inactive Users'){ ?>
                                            <td class="text_center"><input type="checkbox" data-id="<?=$post->id;?>" class="block_user" name="block_user" value="1" <?=($post->is_blocked == 1)?'checked':'';?>></td> 
                                    <?php } ?>
                                            <td class="text_center"><?= date('F d, Y ', strtotime($joined_at)) ?></td> 
...
<script>
    $(document).ready(function () {
        ...
        $('body').on('change', '.approve_user', function () {
        if(confirm('Do you really want to change approve status for this user?')){
            $this = $(this);
            var id = $this.attr('data-id');
            if($(this).is(':checked')){
                status = 1;
...
//                },
        ...
        $('body').on('change', '.block_user', function () {
        if(confirm('Do you really want to change block status for this user?')){
            $this = $(this);
            var id = $this.attr('data-id');
            if($(this).is(':checked')){
                status = 1;
